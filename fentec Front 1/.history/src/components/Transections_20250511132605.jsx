...
const Transactions = () => {
    ...
    const refetch = (refetch_mode='next',data) => {
        let link_next = data?.next; 
        let link_previous = data?.previous; 
        switch (refetch_mode) {
            case 'next':
                if (link_next){
    ...
    if (prevPageRef.current) observer.observe(prevPageRef.current);
    return () => {
        if (nextPageRef.current) observer.unobserve(nextPageRef.current)
        if (prevPageRef.current) observer.unobserve(prevPageRef.current)
    }
    ...
    return ( 
        <div className="transection-section ">
                            ...
                            <div className="transections custom-overflow" ref={transectionsRef}>
                                ...
                                <div className="end" ref={nextPageRef}>---The End----</div>
                            </div>
                    </div>
     );
}
