...
const WithdrawalRequestsPage = () => {
    ...
    const handleResposne = (data) => {  
      const cancelled_trx = data.cancelled_trx
      console.log('cancelled_trx: ', cancelled_trx);
      if(data.success === "withdrawal cancelled"){
        console.log('data.succes: ', data.success);
        setMockithdrawals(mockWithdrawals.map((with_trx) => {
    ...
    const handleCancelOrApproveWithdrawal = (pin) => {
      setFormData({ ...formData, payment_pin: pin });
      sendExRequest("/account/withdraw-money-requests/", "POST", formData,handleResposne)
    }

    useEffect(() => {
      if(mockWithdrawals){
        setFilteredWithdrawals(mockWithdrawals.filter(
          (w) => w.status === activeTab.toLowerCase()
        ))
      }
    },[mockWithdrawals,activeTab])
        ...
        <div className="cards-container">
          {filteredWithdrawals.map((withdrawal) => (
            <div key={withdrawal.id} className="withdrawal-card hover-effect">
              ...
              <input type="text" value={reason} name="" id={withdrawal.id} placeholder="write the reason for cancelation"
              maxLength={200} onChange={(e) => {setReason(e.target.value)}}/>
            </div>
          ))}
