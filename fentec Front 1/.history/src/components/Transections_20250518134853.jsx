...
const Transactions = () => {
    ...
    const findSum = (creditOrsum,data) => {
        if (creditOrsum == 'credit'){
            ...
            let total = 0 ;
            data?.forEach((item) => {
                let total_credit = item.transection_type === 'deposite' ? true :
                let trx_type = item.transection_type === 'deposite' ? true :
                             item.transection_type === 'refund' ? true : false;
                
                if (item.trx_type === 'credit'){
