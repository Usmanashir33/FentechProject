...
const Transactions = () => {
    ...
    const handleDateSelection =(range) => {
            ...
            let calenderDate = new Date(calenderDate)
            let last = last_day.setDate(calenderDate.getDate() -1)
            console.log('last_day: ', last_day);
            console.log('calenderDate: ', calenderDate);
            if (calenderDate !== 'Invalid Date'){
                let calender_data = transections.filter(item => {
                    const itemDate = new Date(item.trx_date);
                    if ( itemDate <= calenderDate && itemDate > last_day){
                        return item
            ...
            setDisplayFilter(false);
              ...
              switch (range) {
                ...
                case 'thisWeek':
                    let this_weekly = transections.filter(item => {
                        const itemDate = new Date(item.trx_date);
                ...
                case 'thisMonth':
                    let this_month = transections.filter(item => {
                        const itemDate = new Date(item.trx_date);
