    ...
    const findSum = (creditOrsum,data) => {
        if (creditOrsum == 'credit'){
            ...
            data.forEach((trx,index) => {
                console.log('trx: ', trx);
                let trx_amount = Number(trx.amount);
                console.log('trx.amount: ', trx.amount);
                console.log('trx_amount: ', trx_amount);
                if (trx?.transection_type === 'credit' && trx_amount > 0){
                    total += trx_amount;
                }
            })
            return total;
        ...
        if (creditOrsum == 'debi'){
            // loop throgh the filteredTrxs and get the total debits
            let total = 0;
            data.forEach((trx) => {
                let trx_amount = parseFloat(trx.amount);
                if (trx_amount < 0){
                    total += trx_amount;
                }
            })
