...
const WithdrawalPage = () => {
  ...
  const {transections,setTransections,sendRequest} = useContext(liveContext);
  ...
  const getWithdrawalStatus = (data) => { 
    if (data.success === "success") {
      ...
      setTimeout(() => {
        setTransectionStatus(data.success);
          setCurrentUser(data.resp.user)
          setTransections([data.resp.trx,...transections])
      }, 2000);
    }else {
  ...
  const dateSelected = (e) => {
      ...
      if (!e.target.value){ // clear 
        setWithdrawalTrx(transections.resul?.filter((trx) => trx.transaction_type=="Withdraw"))
      }else{
    ...
    useEffect(() => {
      setWithdrawalTrx(transections?.results?.filter((trx) => trx.transaction_type=="Withdraw"))
    },[transections])
    useEffect(() => {
