...
const  useWebSocketHook = () => {
    ...
    const [with_trx,setWithTrx] = useState(null)

    const connectSecket = asyn() => {
        // Initialize the WebSocket connection and assign it to webSocketRef.current
        const token = getToken()
        if (token) {
            webSocketRef.current = new WebSocket(`${config.WS_URL}/fentech-live-server/?token=${token}`,);
