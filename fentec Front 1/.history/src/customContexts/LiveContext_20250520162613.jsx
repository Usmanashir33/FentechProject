    ...
    useEffect(() => {
       if (trx){ // moey trx signal come 
        console.log('trx: ', trx);
            let user = trx.user
            setCurrentUser(user);
            if (trx?.updated_trx){ // if the trx is refund/approval and comes with two trxs refunding and status updating
                if (trx.trx){ // trx is refund + updated trx 
                    return setTransections((prev) =>({
                        prev,
                        results: [
                            trx?.trx,
                ...
                } // its approval request without refund trx 
                return setTransections((prev) =>({
                    prev,
                    results: [
                        ...prev?.results.map((trans) => {
