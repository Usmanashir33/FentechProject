...
const LiveContextProvider = ({children}) => {
    ...
    const updateTrx = (updated_trx) => {
        setTransections(
            transections.map((trans) => {
                return (trans.id == (updated_trx?.id))? (updated_trx) : trans
            })
        )
    }
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if(withdrawalRequests){  // if withdrawalRequests changed count the unread ones 
            let unread = withdrawalRequests.filter(({read_by_approver,status}) => (read_by_approver === false && status === 'pendin')).length
